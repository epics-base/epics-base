...
Two device support modules are provided for retrieving values into
...
C<read_lsi()> calls C<dbGetLink()> to read the value into VAL,
truncating at SIZV characters.

The INP link is a standard input link.

=head3 getenv

This module reads the named environment variable into VAL on each
process event. C<INST_IO> addressing is used on the INP link with the
environment variable name as the parameter, e.g.
C<< field(INP, "@HOSTNAME") >>. See L<Built-in Device Support|devSoft/getenv>
for details.
