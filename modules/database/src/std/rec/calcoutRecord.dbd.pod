...
Two soft device support modules are provided for writing the output
of records not related to actual hardware devices. The OUT link must
be a CONSTANT, DB_LINK, PV_LINK, or CA_LINK.

=head3 Soft Channel

The C<Soft Channel> module writes the current value of OVAL through
the record's OUT link.
