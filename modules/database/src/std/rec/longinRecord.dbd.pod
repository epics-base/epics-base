...
must be a CONSTANT, DB_LINK, PV_LINK, or CA_LINK.
...
type is PV_LINK, then dbCaAddInlink is called by C<init_record()>.

C<read_longin> calls recGblGetLinkValue to read the current value of
VAL. See L<Soft Input> for more information.

If the return status of C<recGblGetLinkValue> is zero then read_longin
sets UDF to FALSE. read_longin returns the status of
C<recGblGetLinkValue>.
