...
or via C<dbPutField()> or C<dbPutLink()> requests.
...
input value whenever C<read_longin()> is called. The device support routines are
...
Three soft device support modules are provided for reading values
into records that are not attached to actual hardware. The INP link
must be a standard input link.

There is also support for the EPICS C<generalTime> framework via the
C<General Time> device support, which uses an C<INST_IO> link to
expose framework state such as the time-error counter. See
L<Built-in Device Support|devSoft/General Time> for details.

=head3 Soft Channel

The C<Soft Channel> device support module places a value directly in
VAL.

If the INP link type is constant, then the constant value is stored
into VAL by C<init_record()>, and UDF is set to FALSE. If the INP link
type is PV_LINK, then C<dbCaAddInlink()> is called by C<init_record()>.

C<read_longin()> calls C<recGblGetLinkValue()> to read the current
value of VAL. See L<Soft Input> for more information.

If the return status of C<recGblGetLinkValue()> is zero then
C<read_longin()> sets UDF to FALSE. C<read_longin()> returns the
status of C<recGblGetLinkValue()>.

=head3 Async Soft Channel

This module is like the previous except that it reads the value using
asynchronous processing that does not complete until the INP target
record's processing has completed.
