...
links or via C<dbPutField()> or C<dbPutLink()> requests. Two soft device support modules
...
at run-time via C<dbPutField()> or C<dbPutLink()>.
...
input value whenever C<read_mbbi()> is called. The device support routines are
...
Three soft device support modules, C<Soft Channel>, C<Raw Soft
Channel>, and C<Async Soft Channel>, are provided for multi-bit binary
input records not related to actual hardware devices. The INP link
must be a standard input link.

=head3 Soft Channel

C<read_mbbi()> always returns a value of 2, which means that no
conversion is performed.

If the INP link type is constant, then the constant value is stored
into VAL by C<init_record()>, and UDF is set to FALSE. VAL can be
changed via C<dbPut()> requests. If the INP link type is PV_LINK, then
C<dbCaAddInlink()> is called by C<init_record()>.

C<read_mbbi()> calls C<recGblGetLinkValue()> to read the current value
of VAL. See L<Soft Input>.

If the return status of C<recGblGetLinkValue()> is zero, then
C<read_mbbi()> sets UDF to FALSE. The status of
C<recGblGetLinkValue()> is returned.

=head3 Raw Soft Channel

This module is like the previous except that values are read into
RVAL, VAL is computed from RVAL, and C<read_mbbi()> returns a value
of 0. Thus the record processing routine will determine VAL in the
normal way.

=head3 Async Soft Channel

This module is like C<Soft Channel> except that it reads the value
using asynchronous processing that does not complete until the INP
target record's processing has completed.
