 ...
 *
 * `DBF_INLINK` and `DBF_OUTLINK` specify link ﬁelds. A link ﬁeld can refer to a signal located in a hardware module, to a ﬁeld located in a database record in the same IOC, or to a ﬁeld located in a record in another IOC. A `DBF_FWDLINK` can only refer to a record in the same IOC. Link ﬁelds are described in a later chapter.
 *
 * `DBF_INLINK` (input), `DBF_OUTLINK` (output), and `DBF_FWDLINK` (forward) specify that the ﬁeld is a link structure as deﬁned in link.h. There are three classes of links:
 *
 *  1. Constant - The value associated with the ﬁeld is a ﬂoating point value initialized with a constant value. This is somewhat of a misnomer because constant link ﬁelds can be modiﬁed via `dbPutField` or `dbPutLink`.
 ...
 * When ﬁrst loaded the ﬁeld is always creates as a PV_LINK. When the IOC is initialized each PV_LINK is converted either to a DB_LINK or a CA_LINK.
 *
