...
access links via C<dbPutField()> or C<dbPutLink()> requests. Two soft device support
...
Since UNRELEASED, the alarm filter time constant (AFTC) is available for the bi record.
...
behavior). Be sure that the bi record is processed at a reasonably fast rate 
