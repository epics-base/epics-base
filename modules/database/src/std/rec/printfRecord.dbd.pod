...
records that have no hardware-specific device support attached.

=head3 Soft Channel

This module writes the formatted output through the record's OUT link.

C<write_printf> calls C<dbPutLink> to write the value. The OUT link is
a CONSTANT, DB_LINK, PV_LINK, or CA_LINK.

=head3 Async Soft Channel

This module is like the previous except that it writes the formatted
output using asynchronous processing that does not complete until the
target record's processing has completed.

=head3 stdio

This module writes the formatted output, followed by a newline, to the
standard output, standard error, or IOC error log. C<INST_IO>
