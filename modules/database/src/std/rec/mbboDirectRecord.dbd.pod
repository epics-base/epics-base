...
output direct records not related to actual hardware devices. The OUT
link type must be a CONSTANT, DB_LINK, PV_LINK, or CA_LINK.
...
write_mbboDirect calls recGblPutLinkValue to write the current value
of VAL.
...
This module is like C<Soft Channel> except that it writes the value
