...
characters (which can be bypassed by using
L<Long String Input|lsiRecord/long-string-input-record-lsi>).
Several device support routines are available, all of which are soft
