...
run-time via C<dbPutField()> or C<dbPutLink()>.
...
input direct records not related to actual hardware devices. The INP
link must be a standard input link.

=head3 Soft Channel

For this module, read_mbbiDirect always returns a value of 2, which
...
C<read_mbbiDirect()> calls C<recGblGetLinkValue()> to read the
...
C<read_mbbiDirect()> sets UDF to FALSE. The status of
C<recGblGetLinkValue()> is returned.

=head3 Raw Soft Channel

This module is like the previous except that values are read into
RVAL, VAL is computed from RVAL, and C<read_mbbiDirect()> returns a
value of 0. Thus the record processing routine will determine VAL in
the normal way.
