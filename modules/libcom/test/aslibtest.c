...
static const char method_auth_config[] = ""
    ...
    "	RULE(1, RPC) {\n"
    "		UAG(bar)\n"
    // A comma-separated METHOD list accepts any of the named methods and is
    // equivalent to writing each on its own METHOD() line. Here only "x509"
    // matches the test client; "ca" and "anonymous" are additional accepted
    // alternatives that exercise multi-element list parsing.
    "		METHOD(\"x509\",\"ca\",\"anonymous\")\n"
