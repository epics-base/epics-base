...
Channel>, and C<Async Soft Channel>, are provided for analog input
records not related to actual hardware devices. The INP link type
must be either CONSTANT, DB_LINK, or CA_LINK.

=head3 Soft Channel

The C<Soft Channel> device support reads the value directly into VAL,
so no conversion is performed (C<read_ai()> returns 2). If the INP
link type is CONSTANT, the constant value is stored in VAL by
C<init_record()> and UDF is set to FALSE. If the INP link type is
PV_LINK, C<dbCaAddInlink> is called by C<init_record()>.

C<read_ai()> calls C<dbGetLinkValue> to read the current value of VAL.
See L<Soft Input> for details.

=head3 Raw Soft Channel

This module is like the previous except that values are read into
RVAL, allowing the record's conversion stage (linear or breakpoint
table) to run. C<read_ai()> returns 0, so the record support computes
VAL from RVAL in the normal way.
