...
SUPPRESS\_WARNINGS flag is set it will no longer include any `,undefined`
...
settings that would normally appear in Base/configure/CONFIG\_SITE can now
be put in a locally created base/configure/CONFIG\_SITE.local file instead
...
from getting an alarm severity higher than INVALID\_ALARM. It is still possible
...
Launchpad bugs have also been fixed in this release:
  ...
  - [\#1440186](https://bugs.launchpad.net/epics-base/+bug/1440186) Crash due to a too small buffer being provided in
...
controlled in the files configure/os/CONFIG\_SITE.Common.windows-x64-static and
configure/os/CONFIG\_SITE.Common.win32-x86-static by setting the variable
...
the version of EPICS Base it was built against (EPICS\_VERSION\_...) and its build
...
There are also 3 new environment parameters (EPICS\_BUILD\_...) available that
...
RSRV now honors EPICS\_CAS\_INTF\_ADDR\_LIST and binds only to the provided list
...
and 10.5.3.1/24, setting "EPICS\_CAS\_INTF\_ADDR\_LIST='10.5.1.1 10.5.2.1'" will
accept traffic on the .1.1 and .2.1, but ignore from .3.1

RSRV now honors EPICS\_CAS\_IGNORE\_ADDR\_LIST and ignores UDP messages received
from addresses in this list.

Previously, CA servers (RSRV and PCAS) would build the beacon address list
using EPICS\_CA\_ADDR\_LIST if EPICS\_CAS\_BEACON\_ADDR\_LIST was no set. This is no
...
must be listed in EPICS\_CA\_ADDR\_LIST for clients and EPICS\_CAS\_INTF\_ADDR\_LIST
for servers (IOCs should set both). For example:
"EPICS\_CAS\_INTF\_ADDR\_LIST='224.0.2.9' EPICS\_CA\_ADDR\_LIST=224.0.2.9".
...
This fixes
[Launchpad bug \#1581212](https://bugs.launchpad.net/epics-base/+bug/1581212), thanks to Till Strauman and Dehong Zhang.
...
([Launchpad bug
\#1563191](https://bugs.launchpad.net/bugs/1563191)).
...
the output file to be specified setting COMMON\_ASSEMBLIES (arch independent)
...
The IOC has not called the get\_value() routine in the RSET for a very long
...
DBF\_ULONG to allow an ROFF setting of 0x80000000 to work properly.
...
change has been made to the inclusion of the \&lt;top\>/configure/RULES\_BUILD
files.

Two new file types can also be provided in a module's cfg directory. Files
named TOP\_RULES\* will be included by the top-level Makefile of other modules
that refer to this module; files name DIR\_RULES\* will be included by all
...
The new cfg/DIR\_RULES\* file inclusion was designed to permit new recursive
...
when doing this. Similar rules may also be defined in the cfg/TOP\_RULES\* and/or
cfg/DIR\_RULES\* files, but these should only state prerequisites and not directly
...
Windows tools at the time. All uses of PATH\_FILTER in Base have now been
...
variable name used in the rules has been changed from MSI to MSI3\_15. Sites that
use a modified version of msi must provide support for both the `-D` and
`-o outfile` options, and should then point the MSI3\_15 variable in
their applications' CONFIG\_SITE files to that updated executable.
...
$(INSTALL\_LOCATION)/dbd without expanding it.
...
$(INSTALL\_LOCATION)/dbd without expanding any of its include statements.
...
the SRC\_DIRS list, be specified by their full pathname, exist in the install dbd
...
in configure/CONFIG\_SITE or related files, the Posix implementation of the
libCom `osiSpawnDetachedProcess()` routine will switch the child process
to use the normal SCHED\_OTHER (non real-time) scheduler before executing the
...
read from the standard environment parameter EPICS\_CAS\_INTF\_ADDR\_LIST.
...
with the macro EPICS\_UNUSED. On gcc and clang this will expand to
...
now allowed by setting the variable SUBST\_SUFFIX in a configuration directory
CONFIG\_SITE file or in a Makefile before the include $(TOP)/configure/RULES
line. The default for SUBST\_SUFFIX is `.substitutions`
...
event to be sent to all record subscribers who have asked for DBE\_PROPERTY
...
On Posix platforms this uses the pthread\_spinlock\_t type if it's available and
the build is not configured to use Posix thread priorities, but otherwise it
falls back to a pthread\_mutex\_t. On the UP VxWorks and RTEMS platforms the
...
##### mbboDirect
  ...
  - If the MASK field is set in a database file, it will not be over-written
    when the record is initialized. This allows non-contiguous masks to be set,
    although only the device support actually uses the MASK field.
  - After the device support's init\_record() routine returns during record
...
in configure/os/CONFIG\_SITE.linux-x86.win32-x86-mingw and add win32-x86-mingw to
the CROSS\_COMPILER\_TARGET\_ARCHS variable in configure/CONFIG\_SITE or in
configure/os/CONFIG\_SITE.linux-x86.Common.
...
See rtems\_init.c and setBootConfigFromNVRAM.c in src/libCom/RTEMS.
...
record, the record support code now posts DBE\_VALUE and DBE\_LOG events for that
...
numbers 1-255. The EVNT field is now a DBF\_STRING. The `post_event()` API
...
To debug build issues assocated with dependency files, use the command `make --debug=m` which tells GNUmake to display information about what it is doing
...
was moved to src/ca/client/. ca\_test is now in src/ca/client/test/.
