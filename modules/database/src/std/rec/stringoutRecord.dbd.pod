...
PV_LINK then C<dbCaAddInlink()> is called to create a channel access link.
...
If PACT is FALSE and OMSL is CLOSED_LOOP, C<recGblGetLinkValue()> is called to read
the current value of VAL. See L<Soft Output>.
If the return status of C<recGblGetLinkValue()> is zero then UDF is set to FALSE.
...
a new value whenever C<write_stringout()> is called. The device support routines are
...
Three device support modules are provided for writing values from
records that are not attached to actual hardware.

=head3 Soft Channel

The C<Soft Channel> device support module writes the current value of
VAL through the record's OUT link. The OUT link is a standard output
link.

=head3 Async Soft Channel

This module is like the previous except that it writes the value
using asynchronous processing that does not complete until the OUT
target record's processing has completed.

=head3 stdio

This module writes the record's VAL field, followed by a newline, to
the standard output, standard error, or IOC error log. C<INST_IO>
addressing C<@stdout>, C<@stderr>, or C<@errlog> is used on the OUT
link to select the destination. See L<Built-in Device Support|devSoft/stdio>
for details.
