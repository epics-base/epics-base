...
mbbo value whenever C<write_mbbo()> is called. The device support routines are
...
output records not related to actual hardware devices. The OUT link
must be a standard output link.
