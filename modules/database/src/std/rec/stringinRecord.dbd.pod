...
a new ASCII string value whenever C<read_stringin()> is called. The device support
...
Five device support modules are provided for reading string values
into records that are not attached to actual hardware.

=head3 Soft Channel

The C<Soft Channel> module reads a value directly into VAL through
the record's INP link. The INP link is a standard input link.
...
=head3 Soft Timestamp

This module reads the record's own timestamp formatted by C<strftime>.
The C<INST_IO> INP parameter is the format string. See L<Built-in
Device Support|devSoft/Soft Timestamp> for details.

=head3 General Time

This module exposes properties of the EPICS C<generalTime> framework
(provider names). The C<INST_IO> INP parameter selects which property.
See L<Built-in Device Support|devSoft/General Time> for details.

=head3 getenv

This module reads the named operating-system environment variable
into VAL on each process event. The C<INST_IO> INP parameter is the
environment variable name. See L<Built-in Device Support|devSoft/getenv>
for details.
