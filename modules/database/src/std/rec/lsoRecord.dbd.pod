...
This module writes the current value of VAL through the record's OUT
link.

C<write_lso> calls C<dbPutLink> to write the value. The OUT link is a
CONSTANT, DB_LINK, PV_LINK, or CA_LINK.

=head3 Async Soft Channel

This module is like the previous except that it writes the current
value using asynchronous processing that does not complete until the
