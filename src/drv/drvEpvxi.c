...
)
{
	...
	prealloc = FALSE;
	if(!pvxie->la_mapped){
		status = vxi_count_dc_devices(pvxie, &nDC);
		if(status){
			return;
		}
		if(nDC < 1){
...
)
{
	...
	/*
	 * look inside the range thats mapped first
	 */
	if(pvxie->la_mapped){
		for(la=pvxie->la_low; la<=last_la; la++){
	 ...
	 */
	peak=0;
	for(la=hla; la>=max(VXI_RESMAN_LA+1,first_la); la--){
