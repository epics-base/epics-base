...
sure that SVL is a standard input link. It also reads SGNL from SVL. If SVL
is not a standard input link, an error is generated.

=head4 read_histogram

  read_histogram(*precord)

This routine is called by the record support routines. It reads SGNL from SVL.

=head3 Device Support For Soft Records

Only the device support module C<Soft Channel> is currently provided,
though other device support modules may be provided at the user's
site.

=head3 Soft Channel

The C<Soft Channel> device support routine reads SGNL from SVL. SVL
