...
FALSE. If DOL type is a PV_LINK then C<dbCaAddInlink()> is called to create a channel
access link.
...
return status of C<recGblGetLinkValue()> is zero then UDF is set to FALSE.
...
value whenever C<write_longout()> is called. The device support routines are
...
Two soft device support modules are provided for writing values from
records that are not attached to actual hardware. The OUT link must
be a standard output link.
...
C<write_longout()> calls C<recGblPutLinkValue()> to write the current
value of VAL.
